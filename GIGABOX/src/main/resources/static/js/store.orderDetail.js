$(function(){
	...
	//사용 완료 처리
	$('#status_use').click(function(){
		let choice = confirm('해당 상품을 사용완료 처리하시겠습니까?');
		if(choice){
			let orders_num = $('#orders_num').text();
			$.ajax({
				url:'admin_statusUse.do',
				type:'post',
				data:{orders_num:orders_num,orders_status:2},
				dataType:'json',
	...
	$('#status_cancel').click(function(){
		...
		if(choice){
			let orders_num = $('#orders_num').text();
			$.ajax({
				url:'admin_statusCancel.do',
				type:'post',
				data:{orders_num:orders_num,orders_status:4},
				dataType:'json',
				success:function(param){
					if(param.result == 'wrongAccess'){
